...
async function isStrategyOverexposed(strategy) {
    try {
        const currentCR = ethers.BigNumber.from(
            (await strategy.currentCollateralRatio()).toString()
        );
        const minForRebalance = ethers.BigNumber.from(
            (await strategy.getCollateralRatioTargets())[1].toString()
        );
...
async function hasEPSDecreased(store, strategy) {
    ...
    try {
        ...
        return {
            ...
            hasEPSDecreased: ethers.BigNumber.from(prevEPS.toString()).lt(
...
async function checkAlertChannelsExist(client) {
    ...
    let alertChannels = notificationChannels.filter(
        (channel) => channel.name === 'seamless-alerts'
    );
